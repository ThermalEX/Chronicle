...
当前版本为 **1.3.2**。支持 Steam 存档识别、简体中文与英语界面、自动备份、自动上传、托盘后台运行与多源云端同步，可在应用内检查更新。

## 功能

- 扫描 Steam 多个游戏库的存档来源，按账号昵称分组，勾选后批量添加；扫描结果本地缓存，可选择手动刷新。
- 支持简体中文与英语，欢迎页和设置中均可切换，立即生效并保存选择。
- 首次使用提供语言选择与动态问候、主题预览及分步教程，涵盖创建存档、自动化、快照恢复、星标、同步和 Steam 识别；可跳过，也可在关于页重新开始。
...
点击顶部工具栏的 Steam 图标，打开「游戏存档识别」弹窗，自动识别 Windows 上的 Steam 安装目录和多个游戏库。也可以手动选择包含 `steamapps` 的 Steam 目录。扫描结果和时间保存在本机，再次打开直接显示上次结果，需要时点击「刷新扫描」。
...
教程支持主题与浅色／深色实时预览，随后引导配置云端或仅本地使用、创建存档、备份、恢复、星标与同步。最后一步可直接打开 Steam 存档识别窗口，打开不会自动扫描。日间／夜间切换按钮位于整个页面右下角。
