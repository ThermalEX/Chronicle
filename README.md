...
- 支持多同步源：可分别启用、暂停和检测每个来源；一次同步会发送到全部已启用来源。
- 支持 OpenDAL 服务模板，包括 S3 / 兼容 S3、Backblaze B2、Azure Blob、Google Cloud Storage、阿里云 OSS、腾讯云 COS、华为云 OBS、火山引擎 TOS、OpenStack Swift、GitHub 和 WebDAV。
- 云端仓库提供预览、刷新、覆盖上传/下载、单条目同步和删除；切换来源会自动刷新。
- 云端设置在关闭前提示保存，配置或同步开关变动后会要求测试；机密仍只存 Windows 凭据管理器。
- 时间节点增加备注、详情面板、单节点恢复/同步/删除，以及更稳定的窄窗口布局。
- 统一通知颜色和层级，通知不会被设置或确认窗口遮挡。

### v1.1.0-beta（预发布）

**创新**

- 将原有 GitHub / WebDAV 同步扩展为基于 Apache OpenDAL 的通用云端适配，并引入多同步源模型。
- 引入读写、列举、删除和临时对象清理的来源检测，以及启动时云端健康状态。
- 将云端机密与资料库 JSON 分离，改由 Windows 凭据管理器保存。

**测试重点**

- 各对象存储服务的字段映射、权限范围和端点兼容性。
- 配置变更、凭据重新测试、暂停来源和多源并行同步的状态一致性。
- 远端目录约束、失败重试、临时对象清理及错误提示。

### v1.0.0

- Windows 桌面应用，支持安装版和便携版资料库。
- 将文件或文件夹加入资料库，创建可恢复的 `.7z` 时间节点。
- 支持分类、标签、搜索、时间排序、保留策略和回收站。
- 支持查看内容变更、恢复任意节点、打开本地压缩包和记录快照备注。
- 提供诊断信息、全局快捷键、日间/夜间模式与主题配色。
- 提供 GitHub Repository 和 WebDAV 同步，以及远端覆盖上传/下载。
