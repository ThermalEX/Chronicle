...
import { cloudSettings, type CloudSettings } from "../services/settings";
...
function save(): void {
  Object.assign(cloudSettings, draft);
  emit("saved");
  emit("close");
}
...
<template>
  <div class="dialog-backdrop" @click.self="emit('close')">
    <section class="cloud-dialog" role="dialog" aria-modal="true" aria-labelledby="cloud-title">
      ...
      <footer><button class="cancel-button" @click="emit('close')">取消</button><button class="save-button" @click="save">保存云端设置</button></footer>
    </section>
  </div>
</template>
...
input[type="text"], input[type="url"], input[type="password"], select { width: 100%; height: 36px; padding: 0 10px; color: #263431; background: #f8faf9; border: 1px solid var(--border-2); border-radius: 6px; font-size: 11px; }
