...
import { appSettings, resetAppSettings, type AppSettings } from "../services/settings";
      ...
      <div class="settings-layout">
        ...
        <main>
          ...
          <section v-else-if="activeSection === 'backup'" aria-labelledby="backup-title">
            ...
            <div class="setting-group">
              <label class="setting-row select-row"><span><b>新存档默认分类</b><small>添加后仍可单独修改分类</small></span><select v-model="draft.defaultCategory"><option>未分类</option><option>游戏</option><option>创作</option><option>工作</option><option>配置</option></select></label>
              ...
              <label class="setting-row"><span><b>每个存档保留版本</b><small>超出数量后按时间清理最旧的普通备份</small></span><input v-model.number="draft.retentionCount" class="number-input" type="number" min="1" max="999" /></label>
            </div>
          ...
          <section v-else aria-labelledby="about-title">
            ...
            <dl class="about-list"><div><dt>存储引擎</dt><dd>Rust · ZIP · SHA-256</dd></div><div><dt>桌面框架</dt><dd>Tauri 2 · Vue 3</dd></div><div><dt>许可证</dt><dd>尚未指定</dd></div></dl>
            <a href="https://github.com/ThermalEX/Chronicle" target="_blank" rel="noreferrer">查看 GitHub 仓库</a>
          </section>
        </main>
      </div>

      <footer><button class="reset-button" @click="reset"><RotateCcw :size="15" />恢复默认设置</button><div><button class="cancel-button" @click="emit('close')">取消</button><button class="save-button" @click="save">保存设置</button></div></footer>
    </section>
  </div>
</template>
